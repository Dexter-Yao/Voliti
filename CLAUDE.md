...
| 系统架构 | `docs/03_Architecture.md` | 系统结构、组件关系、数据流、部署与测试视图 |
| Witness Card | `docs/04_Image_Generation.md` | Witness Card 图片生成技术、统一视觉体系、卡片结构 |
| 基础设施里程碑 | `docs/05_Runtime_Foundation_Milestone.md` | 当前运行时基础设施里程碑的目标状态、工作流、分期与验证矩阵 |
| 运行时契约 | `docs/06_Runtime_Contracts.md` | **共享持久化真相、Store、session、A2UI、错误、记忆分层、可观测性边界** |
| GTM 洞察 | `docs/07_Go_To_Market_Insights.md` | 走向市场的战略思考与定位分析 |
| Runtime Harness | `docs/08_Runtime_Harness_Control_Plane.md` | DeepAgent 复用边界、Session Profile、提示词装配、记忆生命周期 |
...
| 2026-04-12 | 文档体系梳理：AGENTS.md 与 CLAUDE.md 同步维护（Codex 消费）；清理历史归档（hackathon）；修复 07 编号冲突（Runtime Harness → 08）；补全文档引用表（GTM/Harness/设计参考/知识库） |
