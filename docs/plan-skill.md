...
| 工具层 | `backend/src/voliti/tools/plan_tools.py` | 5 个 tool + `_execute_plan_tool` helper + archive-first 写入 + current 自愈读取 + dashboardConfig 同步 |
...
| Skill tool 桥接 | `backend/skills/coach/plan/tool.py` | re-export `plan_tools` 的 5 个 tool；`TOOLS = [...]` 列表；`_load_skill_tools` 动态装载 |
| HTTP endpoint | `backend/src/voliti/http_app.py` · `GET /plan-view/{user_id}` | 前端单一派生入口；async 读取 Store → 调 `compute_plan_view` → 返回 `{plan, plan_view}` |
...
| 前端 API wrap | `frontend-web/src/app/api/me/coach-context/route.ts` | 调 `/plan-view` + batch 读 dashboardConfig / copingPlans / markers → 返回顶层字段 |
...
4. **状态性 vs 结构性分流** —— 工具按 diff 字段自动判定：仅 `current_week` / `revised_at` 变化 → 就地改；`target` / `chapters` / `linked_*` / `status` / `supersedes_plan_id` 任一变化 → 归档 + `version++`。Coach 不需要判断"这算不算重大修订"。
...
派生字段（不入 Store）：`week_index`、`active_chapter_index`、`plan_phase`、`week_freshness.{level, days_since_update}`、`day_progress`、`days_left_in_chapter`、`day_template[3]`、`watch_list[]`、`map_state.events[*].urgency`。
...
`/profile/dashboardConfig` 与 Plan 的关系收口为**单向派生 + write-through**：
...
- **保留策略**：已有 `dashboardConfig.north_star` 与 `user_goal` 保留（onboarding placeholder 优先）；只重写 `support_metrics`
...
选择 write-through 而非 API 实时派生：现有 MirrorPanel 通过 dashboardConfig 读 north_star + support_metrics 的代码路径不动，改动最小。未来如果 dashboardConfig 其他字段也需要从 Plan 派生，再考虑把 dashboard 整体改为派生 record。

## 9. 前端呈现

- **MirrorPanel 主流程**（`plan_phase = "in_chapter"`）：Chapter 号 + 用户 identity_statement + target_summary + chapter.title + milestone + Journey progress bar + North Star + Support metrics + LifeSign + 事件流 + Witness Card 画廊。Chapter 号旁显示 freshness 时间回声（`stale`/`very_stale` 时）。
...
| dashboardConfig 同步失败导致 Mirror 指标滞后 | fail-open + 下次结构性修改重试；MirrorPanel 不 crash（只是过时） |
...
- **共建 overlay 撤回**：删除 `fan_out_plan_builder` 工具 + 前端 plan-builder 子目录；其他 4 个 tool 保留，Coach 靠对话完成修订。
