...
当前系统里，候选信号已经有一个**部分实现的雏形**，但还不是完整闭环。
...
6. 将该 brief 注入当前 invocation 的 system prompt
...
4. 完整的持久化、promotion、过期与重算协议尚未形成
...
| `Candidate Distillation` | markers、coach memory、ledger、近期行为片段 | candidate signals | 当前主要由 `JourneyAnalysisMiddleware` 产出到 prompt；少量 freshness marker 落在 `/user/derived/...` | 部分已有 |
| `Injection` | 权威语义、候选信号、archive evidence、runtime evidence | 当前 invocation 的模型输入 | `system_prompt` + `memory` + middleware 注入 | 已有但规则未收口 |
| `Consolidation` | 本轮候选信号、显式确认的稳定结论、completion 结果 | 权威语义更新、候选层 cleanup、freshness marker | 当前应落在 `after_agent` policy | 尚未正式化 |
| `Observation` | 上述全链路中的关键决策与结果 | trace、event、eval sample | logs / trace / metrics | 已有基础，未完全收口 |
...
3. archive 结果只能作为 evidence，不能直接 promotion，这条规则尚未代码化。
4. 冲突、过期、重算与淘汰缺少统一策略。
5. `after_model` 与 `after_agent` 对记忆工作的职责边界尚未明文化。
...
4. `Consolidation` 当前明确落在六个原子阶段中的 `after_agent`。
...
| `after_model` | 提取本轮候选信号、判断是否需要会后整理 | 直接 promotion 到权威语义 |
| `after_agent` | 执行会话结束后的整理、promotion 判定、freshness 更新、candidate cleanup | 重新装配本轮 prompt |

明确约束：

1. `before_model` 负责装配当前输入，不负责会后整理。
2. `after_model` 负责“提炼候选”，不负责“最终落账”。
...
6. 将 brief 注入当前 invocation 的 system prompt
...
3. 生成的 brief 当前只进入本轮 prompt，没有形成统一 candidate signal store。
...
2. `after_agent` consolidation policy 负责判断该 brief 只用于本轮、仅更新 freshness，还是可以提炼成更稳定的 candidate artifact。
...
4. 候选信号的 producer / consumer / freshness 语义写清
...
5. `after_agent` consolidation 规则
