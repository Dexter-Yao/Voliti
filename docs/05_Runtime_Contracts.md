...
3. 共享 fixture 源的正式目录为仓库根级 `tests/contracts/fixtures/`，不归属 backend、客户端或 `eval` 任一端私有测试目录。
4. 强格式 JSON 路径的 fixture 必须与 `backend/src/voliti/contracts/` 下的 Pydantic 模型保持同步（由 `test_fixture_passes_contract_model` 参数化守护）。
...
**子 namespace**：
- Witness Card：`("voliti", user_id, "interventions")`
- Plan archive：`("voliti", user_id, "plan_archive")`，key 格式 `{plan_id}_v{version}.json`；`/plan/current.json` 是 archive 最新版本的派生指针，archive 为权威真相，current 写入失败自愈。详见 `docs/plan-skill.md § 2` 核心设计原则。
...
2. **路径 / key 常量来源**
   - 负责定义 profile、chapter、timeline、coach memory、coping plans 等正式路径。
...
JSON 型强格式 Store 路径必须经过 Pydantic 契约模型做双端 fail-closed 守护。当前权威模型位于 `backend/src/voliti/contracts/__init__.py`，与路径一一对应：

| 路径 | Pydantic 模型 |
|---|---|
| `/plan/current.json` | `PlanDocument`（含 6 条 `@model_validator` 跨字段约束） |
| Plan archive 子 namespace 下 `{plan_id}_v{n}.json` | 同 `PlanDocument`；archive 为权威真相 |
| `/timeline/markers.json` | `MarkersRecord` |
| `/profile/dashboardConfig` | `DashboardConfigRecord` |

Plan 相关写入不走通用 `store_write_validated`，而是 `plan_tools.py` 的 `_execute_plan_tool` helper：archive-first 写入 + current 自愈 + 结构性/状态性 diff 归档判定 + dashboardConfig 同步。Plan 相关读取统一走 `plan_runtime.py`：负责 current/archive 自愈、跨 `plan_id` 权威选择，以及 `no_plan` / `plan_data_corrupted_recovered` / `plan_data_corrupted_unrecovered` 三种降级语义。Pydantic 校验失败时返回 `format_plan_write_error` 格式化的 actionable 中文错误，由 Coach 决策修正。所有 Planner tool 结果统一封装为 JSON 字符串对象，至少包含 `action` / `status` / `write_kind` / `summary` / `plan_id` / `version` / `archive_keys` / `warnings` 八个键；`summary` 继续承担人类可读摘要，其余字段供 agent harness、测试与观测链路稳定分支。详见 `docs/plan-skill.md § 4`。

约束如下：

1. **写入端 fail-closed**：强格式路径的写入必须经过 `store_write_validated(store, namespace, key, data, model_class)`。`ValidationError` 时返回 `(False, 中文错误消息)`，调用方负责将消息封装为 tool result 转交给 Coach，由 Coach 决策是否修正后重试；不得静默降级为未校验写入。
2. **读取端 fail-closed**：结构完整性校验失败时必须抛 `InvalidStoreValueError`，调用方捕获后返回 HTTP 500 或等价错误态，不得把损坏数据静默降级为 `null` 透传。
3. 错误消息为硬编码中文字符串 + 最小合法格式示例；不依赖 LLM 转写，不消耗 Coach token。
4. 前端跨端守护经结构性必要字段检查实现（`assertValidStoreJson`），必填字段列表手工镜像 Pydantic 模型，模型演进时需同步。
5. 候选信号路径与 archive_source 路径不经过此校验层（格式为 Markdown 或自由文本）。
...
当前 backend 实现已按上述最小形态收口（`backend/src/voliti/session_type.py` 的 `SessionProfile` dataclass），会话差异通过同一 profile 入口驱动 `system_prompt`、`memory` 与 middleware 装配。`coaching` 与 `onboarding` 共享同一 `system_prompt_name="coach_system"`，差异由 `SessionTypeMiddleware` 在运行时按 `session_type` 注入 `onboarding.j2` 附加段；`memory_paths` 按会话类型独立配置。

约束如下：

1. backend 负责维护 profile 定义与运行时解释。
2. 客户端只能消费 backend 暴露的稳定会话字段，不复制 profile 逻辑。
...
5. 桌面端在 `onboarding_complete: true` 成功写入前，必须保持 onboarding 专属全屏 surface，不得暴露标准 coaching workspace，也不得预先创建 `coaching` thread。标准 coaching workspace 包括历史栏、Mirror、设置入口，以及面向 `coaching` 的默认 thread 自动选择与挂载。
6. 桌面端 onboarding surface、thread 选择与 `session_type` 必须由同一已解析状态驱动，不得由彼此独立的本地启发式分别决定。
7. 设置页的补采入口属于 `Re-entry`，必须创建 `session_type: onboarding` 的独立 thread 并保持全屏 onboarding surface。
8. `Re-entry` 不得通过清空 `onboarding_complete` 或伪造 reset 达成；其 durable 前提是 profile 仍保持 `onboarding_complete: true`。
...
| 身份键 | `card_id`、`achievement_type`、`linked_lifesign_id`、`chapter_id` | 工具代码硬编码（如 `issue_witness_card` 调用的底层执行器）| 前端去重、LangSmith 聚合 |
...
| `"witness-card"` | 见证卡片 | `issue_witness_card` 调用的底层执行器硬编码 |
| `"plan-builder"` | Plan 共建全屏外壳 | `fan_out_plan_builder` 专用工具硬编码 |
...
- **`issue_witness_card`**：Coach 仅传结构化见证字段；`surface="witness-card"`、`card_id`、`achievement_type` 等 metadata 由底层执行器写入，Coach 同样不传。
- **`fan_out_plan_builder`**：Coach 传 `chapter_index` + `editable_fields` 两个意图参数；`surface="plan-builder"`、`layout="full"` 由工具代码硬编码。Coach 的职责是判断何时共建 + 为数值字段供给 min/max 区间（基于用户画像与 `numeric-guidelines.md`）；无需参与 metadata 写入。
...
3. `plan-builder` 形态走 `PlanBuilderShell` + `PlanBuilderLayout` 全屏 overlay；支持 `text` / `text_input` / `slider` 三种组件；详见 `docs/plan-skill.md § 7, § 9`。
4. 其他键前端不读取，纯观测用途。
...
完整应用边界见 `docs/10_Experiential_Interventions.md`（intervention）与 `docs/plan-skill.md`（plan-builder）。
...
2. Plan、LifeSign、timeline markers。
...
3. `/plan/current.json`
4. `/plan/archive/{plan_id}_v{version}.json`
5. `/coach/AGENTS.md`
...
1. `/derived/...` 候选信号与分析结果，其中 `/derived/briefing.md` 承载日常摘要与 `<user_plan_data>` 只读切片，作为 Coach 每日上下文来源。
2. `/day_summary/{date}.md` 日摘要（≤60 字单句，`archive_source`，日终 Pipeline 生成，无会话日由 Pipeline 自动回填）。
3. `/conversation_archive/{date}.md` 按天会话归档（`archive_source`，日终 Pipeline 写入）。Coach 通过 `grep` 关键词定位日期后 `read_file` 单日文件，禁止一次性加载全部归档。
...
当前实现未引入独立的 `semantic_write_*` 运行时字段。显式确认写入语境由当前对话、工具调用与权威路径三者共同构成；如未来需要新增稳定字段，必须先在本文定义，再落入代码。
...
1. `/profile/...`、`/plan/current.json`、`/plan/archive/{plan_id}_v{version}.json`、`/coach/AGENTS.md`、`/coping_plans_index.md`、`/coping_plans/...`、`/lifesigns.md`、`/timeline/markers.json` 属于 `authoritative_semantic`。
2. `/derived/...` 属于 `candidate_signal`。
3. `/day_summary/...`、`/conversation_archive/...` 属于 `archive_source`。`/plan/archive/...` 虽含 `archive` 字样，但承载的是历史权威 Plan 快照，归 `authoritative_semantic`。
4. `/observability/...` 属于 `observability_only`。
5. 分类同时兼容 backend 视角路径与 `/user/...` 前缀路径，不允许调用方各自维护另一套归一化逻辑。

代码入口：`voliti.semantic_memory.classify_semantic_memory_path(path)` 返回上述六类字面量；配套辅助函数 `is_authoritative_semantic_memory_path` / `is_candidate_signal_path` / `is_archive_source_path` / `is_observability_only_path`。
...
1. 用户触发链路的 `correlation_id` 由客户端在请求边界生成。
...
| 2026-04-18 | §8.5 收口 Witness Card metadata 写入责任：Coach 只调用 `issue_witness_card(...)` 并传结构化字段，`surface="witness-card"` 与身份键由底层执行器写入；Mirror 的 Witness Card 回看以 Store 为唯一真相源 |
| 2026-04-19 | §3 删除 03↔05 分工重复表格，仅保留共享 fixture 约束；§6.2 Store key 列表补 `/chapter/archive/{id}.json` 与 `/coping_plans_index.md`，新增 Witness Card 子 namespace 说明；§6.5 新增强格式路径契约校验层子节（Pydantic 双端 fail-closed、`store_write_validated` / `store_read_validated` 代码入口、四个对应模型），原 §6.5 持久化完成语义顺延为 §6.6；§7.3 补 onboarding 与 coaching 共享 `system_prompt_name` 并由 middleware 分流说明；§7.5 合并条 6-8 消除重复表达；§10.3 权威语义路径补 `/coping_plans_index.md`；§10.3 briefing 措辞 evergreen 化；§10.4 明确 `/goal/archive/` 与 `/chapter/archive/` 归 `authoritative_semantic` 并给出代码入口；§12.1 `correlation_id` 生成方由"iOS"改为"客户端" |
| 2026-04-22 | Planner 真相边界收口：§6.5 补 `plan_runtime.py` 的统一自愈读取与降级语义；§10.3/§10.4 从 `/goal/*` `/chapter/*` 切换到 `/plan/current.json` 与 `/plan/archive/{plan_id}_v{version}.json`；删除未落地的 `semantic_write_*` 运行时字段表述 |
