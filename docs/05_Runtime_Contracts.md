...
Plan 工具层（`plan_tools.py` 的 5 个 tool）不走通用 `store_write_validated`，而是 `_execute_plan_tool` helper：archive-first 写入 + current 自愈 + 结构性/状态性 diff 归档判定 + dashboardConfig 同步。Pydantic 校验失败时返回 `format_plan_write_error` 格式化的 actionable 中文错误，由 Coach 决策修正。详见 `docs/plan-skill.md § 4`。
...
7. `/coach/AGENTS.md`
8. `/coping_plans_index.md`
9. `/lifesigns.md`
10. `/timeline/markers.json`

以下内容不属于权威语义记忆：

1. `/derived/...` 候选信号与分析结果，其中 `/derived/briefing.md` 承载 Goal / Chapter / Process Goals 的预计算摘要，作为 Coach 每日上下文只读来源。
...
1. `/profile/...`、`/goal/current.json`、`/goal/archive/{id}.json`、`/chapter/current.json`、`/chapter/archive/{id}.json`、`/coach/AGENTS.md`、`/coping_plans_index.md`、`/coping_plans/...`、`/lifesigns.md`、`/timeline/markers.json` 属于 `authoritative_semantic`。
2. `/derived/...` 属于 `candidate_signal`。
3. `/day_summary/...`、`/conversation_archive/...` 属于 `archive_source`。`/goal/archive/...` 与 `/chapter/archive/...` 虽含 `archive` 字样，但承载的是历史权威目标/章节快照，归 `authoritative_semantic`。
