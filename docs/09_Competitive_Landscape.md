...
此外，中国政策环境正在催化需求：国家卫健委在 2024-08 启动"体重管理年"，后续发布《体重管理指导原则（2024年版）》，持续强化体重管理方向。政策会推高需求，也会吸引更多医疗化玩家进入。
...
```mermaid
timeline
    title 体重管理产品与AI行为支持演进
    2024-06 : 中国获批长期体重管理司美格鲁肽
    2025-01 : Noom 发布 Body Scan / Future Me
    2025-03 : Oura Advisor 向会员全面开放
    2025-04 : Wondr Advanced 上线 药前/药中/药后支持
    2025-05 : Keep AI教练卡卡升级并上移到一级入口
    2025-06 : Omada 公布 GLP-1 persistence 数据
    2025-08 : Noom 上线 Microdose GLP-1Rx
    2025-12 : WW 发布 GLP-1 一体化平台
    2025-12 : Noom 发布 Proactive Health Microdose GLP-1Rx
    2026-01 : Omada 公布停药后一年维持数据
    2026-01 : WW 扩展口服 GLP-1 接入
    2026-03 : 中国司美格鲁肽专利到期引发降价预期
    2026-04 : ChatGPT Search / Apps / Memory 进一步强化平台入口
```
...
| Simple | 美国 | 直接 | AI-chat 中心的健康教练 App，24/7 AI 教练 Avo 为核心体验 | 部分：有 daily check-in 和个性化 guidance | 部分：持续陪伴但公开复原协议不明 | 部分：个性化随使用深化 | 是：App 私密性高 | 弱：用户反馈中提到语气带羞耻感 | 高：AI 教练是产品中心 | Premium 订阅 | Avo 24/7 教练 + AVO Vision 视觉识别；产品仍以轻断食/营养计划为框架，用户对 food logging 负担和收费感知有不满。与 Voliti 产品形态最接近的美国竞品。 |
...
| 轻牛健康 | 中国 | 相邻（近邻） | AI 驱动科学减脂与健康管理平台，主打 AI 私教全程陪伴 | 部分：数据 + AI 私教整合 | 部分：有复盘但更依赖身体数据而非情境建模 | 是：多模态记录和持续跟踪 | 中：可私用 | 中：有陪伴但也有打卡 | 中：AI 私教是卖点，行为建模深度待观察 | 付费会员 | App Store 持续迭代多模态记录、打卡、复盘、饮食分析。中国市场结构上最接近 Voliti 的产品，但更依赖硬件数据而非复杂生活情境下的行为建模。 |
...
首先，**AI 热量记录/食物识别/饮食建议型产品**并不是真正的直接竞品。像 MyFitnessPal 这类产品的核心单位，是“记录一餐、算一次摄入、看一次趋势”，不是“在你想暴食前 15 分钟把你拉住，或在破功后的第二天让你不继续沉下去”。即便它们在 2026 年加入照片上传、语音记录等 AI 功能，核心仍是**输入与计算效率**，而非**行为脱轨管理**。所以它们会争走用户的记录时间，但未必能直接满足 Voliti 的核心 job-to-be-done。值得注意的是 MyFitnessPal 在 2026-03 收购了 Cal AI，说明"低摩擦拍照记录"正在被大平台标配化——这会抬高用户对"记录必须极轻"的预期，但不改变记录层与行为教练层的本质区别。citeturn19search0turn19search1
...
```mermaid
flowchart TD
    A[生活状态波动<br/>睡眠不足 压力 情绪 出差 加班 酒局 独处] --> B[State 识别]
    B --> C{是否进入高风险窗口}
    C -- 否 --> D[维持日常最小一致性]
    C -- 是 --> E[Plan 预设护栏<br/>场景预演 未来自我对话 如果-那么计划]
    E --> F[Do 关键时刻微介入<br/>替代动作 延迟动作 环境调整]
    F --> G{是否仍然失控}
    G -- 否 --> H[Check 轻量复盘<br/>强化有效策略]
    G -- 是 --> I[48–72小时恢复流程<br/>去羞耻 认知重构 最小修复动作]
    I --> J[Act 写回长期模型<br/>识别重复坑位 更新个人剧本]
    H --> J
    J --> K[下一轮State更早识别]
```
