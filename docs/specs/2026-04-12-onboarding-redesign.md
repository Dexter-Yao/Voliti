...
### Phase 0: Coach 自我介绍（前端硬编码）

**触发**：用户首次进入 onboarding，消息队列为空。

**实现方式**：前端硬编码展示，不经过后端。iOS 由 `OnboardingGreeting.swift` 承载，Web 由 `OnboardingWelcome.tsx` 新增问候文本承载。后端 onboarding prompt 从 Phase 1 开始，假设 Phase 0 已由前端完成。
...
**交互**：不给建议回复，等待用户输入名字。用户输入名字后，前端以 `session_type: "onboarding"` 创建 thread 并发送第一条消息（用户名字），触发后端 onboarding prompt 注入。
...
- 语气温暖、直接，不讨好也不居高临下。
- 前端硬编码确保问候语即时呈现，不依赖后端 round-trip。
...
不维护单独的 progress 记录，从已有 Store 数据的存在性推断当前阶段：

| 数据状态 | 推断阶段 |
|----------|---------|
| 无 profile | 新用户，Phase 0-1 未完成 |
| profile 含 name 但无身份愿景 | Phase 2 未完成 |
| profile 含身份愿景但无场景数据 | Phase 3 未完成 |
| profile 含场景数据但无 dashboardConfig/chapter | Phase 4 未完成 |
| `onboarding_complete: true` | 全部完成 |

中断恢复时，Coach 读取已有数据，从缺失处继续。不需要精确追踪"上次到哪个 Phase"。

#### 三种入口模式（写入 prompt 框架层）

| 模式 | 触发条件 | Coach 行为 |
|------|---------|-----------|
| 新用户 | 无 profile 数据 | 完整 onboarding 流程（Phase 1 → N） |
| 中断恢复 | profile 存在但 `onboarding_complete` 不为 true | 读取已有数据，从缺失处继续 |
| Re-entry 补采 | `onboarding_complete: true` 且从设置页进入 | Coach 主动识别信息缺口，引导用户补充；可基于已有对话和用户画像，提出哪方面信息对成功达标可能有帮助 |

三种模式均通过 `session_type: "onboarding"` 触发 SessionTypeMiddleware 注入。

#### 完成判定的韧性

两层机制，任一满足即标记完成：

1. **主路径**：Store 中 `profile/context.md` 含 `onboarding_complete: true`。
2. **兜底路径**：前端本地 fallback（iOS: `shouldMarkOnboardingComplete`，Web: localStorage）。
...
7. A2UI 组件必须通过 `fan_out` tool call 调用，不得将 A2UI JSON 作为文本输出。
...
| 字段 | 推断逻辑 |
|------|----------|
| 北极星指标 | 减脂场景默认体重（参照 Metrics Governance 规则） |
| 支持指标 1-3 | 基于用户关注点推断，参照 Metrics Governance 的指标类型和单位约定 |
| 身份陈述 | 基于用户描述的愿景提炼，格式："正在[动词短语]的人" |
| Chapter 目标 | 基于用户提到的最具体的近期目标 |
| user_goal | 基于对话中提及的数字目标或身份目标生成摘要 |
| depth_choice | 写入 profile 作为用户档案的一部分 |
...
所有分析数据从已有数据源派生：
- **depth_choice**：写入 `profile/context.md` 作为用户档案的一部分。
- **场景选择**：已写入 profile。
- **对话轮次/时长**：从 onboarding thread 的 messages 直接计算。
- **选项选择分布**：从 thread 中的 A2UI interrupt/resume 记录提取。
- **Witness Card 结果**：从 thread 中的 subagent 调用记录提取。

不需要额外的分析专用存储。补充性事件可通过现有 ledger 通道记录。

## 八、前端变更

### 8.1 硬编码问候语更新

**iOS**：`OnboardingGreeting.swift` 中的 `textZH` / `textEN` 替换为新版 Coach 自我介绍（见 Phase 0）。

**Web**：`OnboardingWelcome.tsx` 改造：
- 展示新版 Coach 自我介绍文本（与 iOS 一致）。
- 用户输入名字后，以 `session_type: "onboarding"` 创建 thread（当前硬编码 `session_type: "coaching"` 需修改）。
- 删除 `localStorage.setItem(ONBOARDING_KEY, "true")` 的提前标记，改为后端 `onboarding_complete: true` 驱动完成判定。

### 8.2 Web 端 Onboarding 流程

当前 Web 端缺少真正的 onboarding 对话流程。需要：

1. `Stream.tsx` 的 `ensureTodayThread` 需根据用户状态决定 `session_type`：
   - 新用户（无 localStorage 完成标记且无 Store profile）→ `session_type: "onboarding"`
   - 设置页 re-entry → `session_type: "onboarding"`
   - 其他 → `session_type: "coaching"`

2. Onboarding 完成后，前端检测到 Store 中 `onboarding_complete: true`，设置 localStorage fallback 并切换到正常 coaching 界面。

### 8.3 Re-entry 引导

Settings 页面的"继续了解我"入口保持不变。收尾阶段 Coach 的引导语确保用户知晓此入口。Re-entry 使用 `session_type: "onboarding"` 创建新 thread。
...
完全重写 `_ONBOARDING_PROMPT`，从脚本式改为框架式。遵循第五节的三层结构设计。Prompt 从 Phase 1 开始（Phase 0 由前端硬编码完成）。Prompt 需覆盖三种入口模式（新用户/中断恢复/re-entry 补采），由框架层的恢复逻辑段落指导 Coach 识别当前模式。
...
在 prompt 框架层新增恢复逻辑段落，指示 Coach 在每次 onboarding 会话开始时读取已有 Store 数据，从缺失处继续（见"中断恢复与入口模式"章节）。不新增额外 Store 路径。
...
| 2026-04-13 | Eng Review：进度追踪改为数据推断（删除 onboarding_progress）；完成韧性三层简化为两层；补充 Web 端 onboarding 流程；Phase 0 保留前端硬编码；简化推断规则表；新增 fan_out tool call 强制约束；明确三种入口模式 |
